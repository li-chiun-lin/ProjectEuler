...
int euler(int n) {
    ...
    for (int i = 2; i <= n; ++i) {
        int sqrtLimit = static_cast<int>(sqrt(i));
        
        // Skip perfect squares, as they have no periodic continued fractions
        if (sqrtLimit * sqrtLimit == i) {
            continue;
        }

        int m = 0, d = 1, a = sqrtLimit;
        int period = 0;

        // Calculate the period of the continued fraction
        while (a != 2 * sqrtLimit) {
            m = d * a - m;
            d = (i - m * m) / d;
            a = (sqrtLimit + m) / d;
